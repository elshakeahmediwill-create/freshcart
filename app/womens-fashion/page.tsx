...
export default function WomensFashionPage() {
  ...
  async function getWomensFashion(page = 1) {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`https://ecommerce.routemisr.com/api/v1/products?page=${page}&limit=100`);
      const filtered = data.data.filter((product: any) => product.category.name === "Women's Fashion");
...
const { toggleWishlist, wishlistItems } = useWishlist(); // سحب الفانكشن والداتا للمفضلة
  return (
    <main className="bg-gray-50 min-h-screen pb-20">
      <section className="bg-[#13b443] py-10 px-6 lg:px-20 text-white">
        <div className="container mx-auto">
          <nav className="text-[10px] mb-4 opacity-90 font-medium uppercase tracking-widest">
            ...
            <span className="font-bold">Women's Fashion</span>
          </nav>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/90 font-medium">Browse products in Women's Fashion</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 mt-8">
        ...
        {/* فحص حالة التحميل لعرض Spinner احترافي قبل تحميل الكروت */}
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 w-full col-span-full">
            <div className="w-12 h-12 border-4 border-[#4FA74F] border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="text-gray-500 text-sm font-medium animate-pulse">Loading products...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {products.map((product: any) => {
              // الحسبة الديناميكية لمعرفة إذا كان منتج الفاشون الحالي مضاف للمفضلة أم لا
              ...
              return (
                <div key={product._id} className="group bg-white border border-gray-100 rounded-2xl p-4 relative hover:shadow-xl transition-all duration-300 flex flex-col">
                  ...
                  {/* تعديل مكان الأيقونات هنا: right-4 بدلاً من left-4 */}
                  <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
                    <button 
                      onClick={() => toggleWishlist(product)}
                      className={`p-2 shadow-sm rounded-full transition-colors border ${
                        isFav ? "bg-red-50 text-red-600 border-red-100" : "bg-white text-gray-400 hover:text-[#d31919] border-gray-50"
                      }`}
                    >
    ...
    >
      <Eye size={18} />
        ...
        onClick={()=>addToCart(product)}
         // السطر ده هو اللي بيخلي الرقم يزيد
      ...
      >
        ...
        <div className="flex justify-center items-center gap-3 mt-16">
          <button 
            onClick={() => getWomensFashion(currentPage - 1)}
